...
def build_weekly_report_docx(
    ...
    rows = [
        ...
        ("Margin 부족으로 보류(세부유형 미확정)", f"{margin_hold_count:,}건"),
        ("균열(D1) 미검출", f"{d1_miss_count}건"),
        ("용입불량(D4) 미검출", f"{d4_miss_count}건"),
    ]
    for k, v in rows:
...
def weekly_report_bytes(thresholds):
    ...
    recommendation = (
        ...
        "※ 기공(D2) 자동배출 로직은 이번 버전(D1/D4 개별 margin 방식)에 아직 반영되지 않아 "
        "전량 사람 확인으로 분류됩니다 — 별도 반영 필요."
    )
    ...
    docx_bytes = build_weekly_report_docx(
        ...
        margin_hold_count=margin_hold_count, d1_miss_count=d1_miss_count, d4_miss_count=d4_miss_count,
