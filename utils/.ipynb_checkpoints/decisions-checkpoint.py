...
헤더가 서로 다르면 같은 CSV에 다른 컬럼 구성이 섞여 깨지므로, 반드시 동일하게 유지할 것.
"""
import csv
...
FIELDS = ["시각", "이미지출처", "AI판정", "AI세부유형", "무결함", "균열(D1)", "기공(D2)", "용입불량(D4)", "검사자결정"]


def log_decision(source, probs, status, label, decision):
