...
# 움직여도 여기 숫자는 안 바뀐다 — 아래 라우팅 규칙도 이 결과를 낸 기본 임계값 기준.
ROUTING_SUMMARY = [
...
def render():
    ...
    with st.container(border=True):
        ...
        st.caption(
            f'라우팅 규칙(기본 임계값 기준): P(무결함)≥{th["nd_confident"]:.2f}→자동통과 · '
            f'D1·D4 동시 {th["attention_t"]:.2f} 이상→균열계열 의심 · '
            f'최고 결함확률≥{th["confident_t"]:.2f}→자동배출 · 그 외→사람확인'
        )
